...
module.exports = function(data, userId, activityData, callback){
    ...
    if (msg == "running" || msg == "before"){
        if  (!!data[msg].cover)  {
            if (msg == "before") {
                data[msg].url =   "/gaming/rotary/" + msg + "?c=" + _random + "&u=" + userId  + "&a=" + activityData["id"];
                ...
                activity.genUserCode(userId, activityData, function(err, code){
                    if (!err) {
                        data[msg].url =   "/gaming/rotary/" + msg + "?c=" + code + "&u=" + userId  + "&a=" + activityData["id"];
                        return callback(err, [{
                            title: data[msg].title
                            ,description: data[msg].summary
                            ,picUrl: data[msg].cover
        ...
        return callback(undefined, [{
            ...
            ,url: "/gaming/rotary/" + msg + "?c=" + _random + "&u=" + userId + "&a=" +  activityData["id"]
        }]);
